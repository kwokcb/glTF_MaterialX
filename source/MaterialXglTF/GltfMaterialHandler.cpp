...
void GltfMaterialHandler::distillDocument(DocumentPtr doc)
{
    ...
    // Perform baking
    // 
    ImageHandlerPtr _imageHandler = GLTextureHandler::create(StbImageLoader::create());

    // Compute baking resolution.
    ImageVec imageVec = _imageHandler->getReferencedImages(doc);
    auto maxImageSize = getMaxDimensions(imageVec);
    ...
    Image::BaseType baseType = Image::BaseType::UINT8; // _bakeHdr ? Image::BaseType::FLOAT : Image::BaseType::UINT8;
    TextureBakerPtr baker = TextureBaker::create(bakeWidth, bakeHeight, baseType);
    baker->setupUnitSystem(_materials);
    ...
    FileSearchPath extendedSearchPath;// = _searchPath;
